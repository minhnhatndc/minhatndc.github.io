...
int main(void)
{
    ...
    uint32_t f_addr = 0x0007f000; // last page start addr
    uint32_t *p_addr = (uint32_t *) f_addr;// pointer to addr
    
    uint32_t val = 486349;
    NRF_LOG_INFO("Erasing page 127");
    nrf_nvmc_page_erase(f_addr);
    NRF_LOG_INFO("Page erased sucessfully");
    NRF_LOG_INFO("Wriing the value to the given addr");
    nrf_nvmc_write_word(f_addr, val);
    // nrf_nvmc_write_words(f_addr, data, sizeof(data)); // write data[5] to flash
    NRF_LOG_INFO("Word is writen to the fash");
