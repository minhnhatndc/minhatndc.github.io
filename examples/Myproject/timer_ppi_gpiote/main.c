...
err_code = nrf_drv_timer_init(&timer0, &timer_cfg, timer_handler);
APP_ERROR_CHECK(err_code);
uint32_t timer_ticks = nrfx_timer_ms_to_ticks(&timer0, 300);
nrf_drv_timer_extended_compare(&timer0, NRF_TIMER_CC_CHANNEL0,timer_ticks, NRF_TIMER_SHORT_COMPARE0_CLEAR_MASK,true);

// Initialize PPI task and event
compare_evt_addr = nrfx_timer_event_address_get(&timer0, NRF_TIMER_EVENT_COMPARE0);
gpiote_task_addr = nrf_drv_gpiote_out_task_addr_get(led1);
