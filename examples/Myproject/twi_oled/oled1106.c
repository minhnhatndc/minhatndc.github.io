...
void oled_WriteData(uint8_t *data, size_t buffer_size)
{
    ...
    memcpy(&tx_buffer[2], data, buffer_size);
    if(i2c_comm_handle_callback != NULL)
    {
...
Oled_Error_t oled_FillBuffer(uint8_t *buffer, uint32_t length)
{
    ...
    {
        memcpy(OLED_FILLBUFFER, buffer, length);
        err_code = OKAY;
    }
    return err_code;
}
...
void oled_init(oled_i2c_handle i2c_comm_handle)
{
    ...
    #if OLED_MIRROR_VERT
    oled_WriteCommand(0xC0);// Mirror veritically
    #else
...
void oled_enable()
{
  nrf_gpio_pin_write(OLEB_ENABLE_PIN, GPIO_PIN_SET);
}
void oled_disable()
{
  nrf_gpio_pin_write(OLEB_ENABLE_PIN, GPIO_PIN_RESET);
