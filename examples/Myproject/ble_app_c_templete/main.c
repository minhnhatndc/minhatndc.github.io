...
//BLE_CUS_C_DEF(m_cus_c);
BLE_CUSS_C_DEF(m_cus_c,2);
 ...
 */
static char const m_target_periph_name[] = "Nordic";      /**< If you want to connect to a peripheral using a given advertising name, type its name here. */
...
static ble_gap_addr_t const m_target_periph_addr =
{
    ...
    .addr      = {0xC9,0xE3,0xF4,0x98,0x08,0x9D}//{0x8D, 0xFE, 0x23, 0x86, 0x77, 0xD9}
};
...
void ble_cus_evt_handler(ble_cus_c_t *p_cus, ble_cus_c_evt_t *p_cus_evt)
{ 
  ret_code_t err_code;
  switch(p_cus_evt->evt_type)
  {
    case BLE_CUS_C_CONNECTED:
    break;
    case BLE_CUS_C_DISCONNECTED:
    break;
    case BLE_CUS_C_DISCOVERY_COMPLETED:
    NRF_LOG_DEBUG("Heart rate service discovered.");
    err_code = ble_cus_c_handle_assign(p_cus,p_cus_evt->conn_handle,&p_cus_evt->params.peer_db);
    APP_ERROR_CHECK(err_code);
    err_code = ble_cus_c_notif_enable(p_cus);
    APP_ERROR_CHECK(err_code);

    break;
    case BLE_CUS_C_NOTIFICATION:
    NRF_LOG_INFO("Received value = %d", p_cus_evt->params.tmp.tmp_value);
    if(p_cus_evt->params.tmp.sleep_intervals_cnt !=0)
    {
    uint32_t count = 0;
    for(uint16_t i =0; i < p_cus_evt->params.tmp.sleep_intervals_cnt;i++)
    {
      count+= p_cus_evt->params.tmp.sleep_intervals[i];
    }
     count = count/ p_cus_evt->params.tmp.sleep_intervals_cnt;
     NRF_LOG_DEBUG("Sleep intervals value = %d", count);
    }
    break;
  }
}
...
/**
 * @brief Temp collector initialization.
 */
 static void tmp_c_init()
 {
  ble_cus_c_init_t cus_init;
  memset(&cus_init,0,sizeof(cus_init));

  cus_init.evt_handler = ble_cus_evt_handler;
  cus_init.error_handler= service_error_handler;
  cus_init.p_gatt_queue = &m_ble_gatt_queue;

  ret_code_t err_code = ble_cus_c_init(&m_cus_c,&cus_init);
  APP_ERROR_CHECK(err_code);
 }
/**
