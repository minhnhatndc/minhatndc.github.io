...
#include "ble_dis_dev.h"
#include "ble_hrs_dev.h"

#include "ble_gps_dev.h"
#include "ble_sleep_dev.h"
#include "ble_temp_dev.h"

#define SAADC_SAMPLE_RATE       50  // Sampling Time for ADC ms
...
BLE_SLEEP_DEF(m_slp,NRF_SDH_BLE_TOTAL_LINK_COUNT);
// YOUR_JOB: Use UUIDs for service(s) used in your application.
static ble_uuid_t m_adv_uuids[] =                                               /**< Universally unique service identifiers. */
{
    {BLE_UUID_SLEEP_SERVICE, BLE_UUID_TYPE_BLE}
};
...
static void heart_rate_meas_timeout_handler(void * p_context)
{
    ...
    if(cnt < 100)
    {
      if(heart_rate >=10) cnt++;
    }else
    {
      cnt = 0;
    }
    
    err_code = NRF_SUCCESS;//ble_hrs_heart_rate_measurement_send(&m_hrs, heart_rate,cnt);
...
static void services_init(void)
{
     ...
     */
      ...
      slp_init.evt_handler = NULL;
      err_code = ble_sleep_init(&m_slp,&slp_init);
      APP_ERROR_CHECK(err_code);
}
...
void saadc_callback_handler(nrf_drv_saadc_evt_t const *p_event)
{
  ...
  if(p_event->type ==  NRFX_SAADC_EVT_DONE)
  {
    ...
    for(i = 0; i < saadc_buffer_length; i++)
    {
      ...
      sprintf((char*)buff,"%f", value);
      ble_sleep_data_send(&m_slp, buff,&len,m_conn_handle);
      ble_sleep_data_count_send(&m_slp, tmp);
  } 
  //*p_value = 10.5;
}
}
...
int main(void)
{
    ...
    CLCD_I2C_WriteString(&LCD,"Hello World");
    saadc_ppi_init();
    saadc_init();
