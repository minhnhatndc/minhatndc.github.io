...
{
  bool check = false;
  uint8_t dataTemp_MSB, dataTemp_LSB;
  for (int i = 0; i < 2 ; i++)
  {
    DS3231->RxTempBuff[i] = 0;
  }
  check = i2c_read_byte(address,DS3231->RxTempBuff,2);
  if(check == false)
  {
    NRF_LOG_INFO("Read Date error");
  }
  if((DS3231->RxTempBuff[0] & 0x80) != 0)
  {
    dataTemp_MSB = DS3231->RxTempBuff[0] | ~((1 << 8) -1);
  }else dataTemp_MSB = DS3231->RxTempBuff[0];

  DS3231->Temp = (DS3231->RxTempBuff[0]) + (DS3231->RxTempBuff[1])/4;
}
